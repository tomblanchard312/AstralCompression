...
def test_mckay_gist_first():
    ...
    start_time = time.time()
    try:
        unpacked_data, data_type, metadata = mckay.unpack_mckay_message_atomized(atomized_stream)
    ...
    try:
        unpacked_binary, binary_type, binary_metadata = mckay.unpack_mckay_message_atomized(atomized_binary)
        ...
        if isinstance(unpacked_binary, bytes) and unpacked_binary.startswith(b'MCKAY_BINARY'):
    ...
    print(f"   Full stream: {len(atomized_stream):,} bytes ({len(atomized_stream) // 32} atoms)")
    print(f"   Truncated: {len(truncated_stream):,} bytes ({atoms_to_keep} atoms)")
    
    try:
        # Try to unpack truncated data
        unpacked_truncated, truncated_type, truncated_metadata = mckay.unpack_mckay_message_atomized(truncated_stream)
    ...
    if os.path.exists(voice_file):
        try:
            ...
            try:
                unpacked_voice, voice_type, voice_metadata = mckay.unpack_mckay_message_atomized(atomized_voice)
                ...
                if isinstance(unpacked_voice, bytes) and unpacked_voice.startswith(b'MCKAY_VOICE'):
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"🚀 Deep Space Transmission System: FULLY OPERATIONAL with proper architecture!")
        print(f"🇨🇦 McKay's Extreme Compression: Ready for deployment with ASTRAL's robust system!")
    else:
