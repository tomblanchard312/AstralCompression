...
def encode_cmd(cmd: dict, key: bytes = None) -> bytes:
    """cmd example:
    ...
    bw = BitWriter()
    # version 1 (3 bits), reserved(5 bits)
    bw.write_bits(1, 3); bw.write_bits(0,5)
    # command id (varint, byte-aligned)
    bw.align_byte()
    out = bytearray()
    out += bytes([ (1 & 0x07) | (0<<3) ])  # store the version header duplicate for simplicity
...
def decode_cmd(b: bytes, key: bytes = None) -> dict:
    # Input validation
    ...
    if name == "SET_MODE":
        ...
        out["mode"] = inv_mode.get(mode, f"M{mode}")
    elif name == "POINT":
        def read24():
            nonlocal pos
            v = b[pos] | (b[pos+1]<<8) | (b[pos+2]<<16); pos+=3
            if v & (1<<23): v -= (1<<24)
            return v
        az = read24()/10000.0
        el = read24()/10000.0
        out.update({"az":az, "el":el})
    elif name == "BURN":
        thr, pos = leb128_decode(b, pos)
        dur, pos = leb128_decode(b, pos)
        out.update({"thruster_id":thr, "duration_ms":dur})
    elif name == "SCHED_WAKE":
    ...
    if key:
        ...
        if len(b) - pos >= 32:
            mac = b[-32:]
            body = b[:len(b)-32]
            ok = hmac.compare_digest(hmac.new(key, body, hashlib.sha256).digest(), mac)
            out["auth_ok"] = ok
        else:
            out["auth_ok"] = False
...
def encode_cmd_batch(batch: dict, key: bytes=None) -> bytes:
    # Input validation
    ...
    flags = 0
    if policy.get("rollback_on_fail"): flags |= 1
    if policy.get("halt_on_error"): flags |= 2
    out = bytearray()
    ...
    for it in items:
        ...
        cmd = it.get("cmd", {})
        body = encode_cmd(cmd, key=None)  # MAC whole batch at the end (optional)
...
def decode_cmd_batch(b: bytes, key: bytes=None) -> dict:
    # Input validation
    if not isinstance(b, bytes):
        raise ValueError("b must be bytes")
    if key is not None and not isinstance(key, bytes):
    ...
    for _ in range(n):
        off, pos = leb128_decode(b, pos)
        ln, pos = leb128_decode(b, pos)
        body = b[pos:pos+ln]; pos+=ln
        items.append({"tai_offset_s": off, "cmd": decode_cmd(body)})
    out = {"policy": {"rollback_on_fail": bool(flags&1), "halt_on_error": bool(flags&2)}, "items": items}
