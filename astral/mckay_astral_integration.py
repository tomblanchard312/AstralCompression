...
class McKayASTRALCompressor:
    ...
    def compress_with_mckay(
        self, data: Union[str, bytes], data_type: str = "AUTO"
    ) -> bytes:
        ...
        header = self._create_mckay_header(
            data_type, len(raw_data), len(compressed_data)
        )

        # Store compression stats
        self.compression_stats = self._calculate_compression_stats(
    ...
    def _create_mckay_header(
        self, data_type: str, original_size: int, compressed_size: int
    ) -> bytes:
        ...
        header.extend(
            data_type.encode("ascii")[:3].ljust(3, b"\x00")
        )  # Data type (3 bytes)
        ...
        checksum_data = header[
            :25
        ]  # Magic(5) + Version(1) + Type(3) + Orig(4) + Comp(4) + Seq(4) + Ratio(4)
    ...
    def pack_mckay_message_atomized(
        self,
        ...
        msgmeta = {
            ...
            "mckay_version": "1.0",
            "original_size": (
                len(data) if isinstance(data, bytes) else len(data.encode("utf-8"))
            ),
            "compressed_size": len(mckay_compressed),
            "compression_ratio": self.get_compression_stats().get(
        ...
        SYMBOL_SIZE = 16
        blocks = [
            mckay_compressed[i : i + SYMBOL_SIZE]
            for i in range(0, len(mckay_compressed), SYMBOL_SIZE)
        ]
    ...
    def unpack_mckay_message_atomized(
        self, atomized_stream: bytes
    ) -> Tuple[Union[str, bytes], str, Dict]:
        """
        ...
        try:
            ...
            if not fountain_decoded or not fountain_decoded[0]:
                raise ValueError(
                    f"Fountain decoding failed: {fountain_decoded[1] if fountain_decoded else 'None'}"
                )
            ...
            if len(fountain_recovered) != payload_len:
                print(
                    f"Warning: Recovered size {len(fountain_recovered)} != expected {payload_len}"
                )
...
class McKayASTRALIntegration:
    ...
    def compress_and_encode(
        self, data: Union[str, bytes], data_type: str = "AUTO", extra_fountain: int = 0
    ) -> bytes:
        ...
        if not ASTRAL_AVAILABLE:
            ...
            self.integration_stats = {
                ...
                "fountain_overhead": 0,
                "total_compression": self.mckay_compressor.get_compression_stats()[
                    "compression_ratio"
                ],
            }
        ...
        try:
            if data_type == "TEXT" and isinstance(data, str):
                ...
                fountain_encoded = self._encode_with_fountain(
                    mckay_compressed, extra_fountain
                )
            ...
            self.integration_stats = {
                ...
                "total_compression": len(
                    data.encode("utf-8") if isinstance(data, str) else data
                )
                / len(fountain_encoded),
            }
...
def test_mckay_astral_integration():
    ...
    try:
        ...
        encoded = integration.compress_and_encode(
            mission_text, "TEXT", extra_fountain=10
        )
    ...
    try:
        ...
        print(
            f"Binary data: {mckay_stats_binary['compression_ratio']:.2f}x compression"
        )
    ...
    large_text = (
        "The Ancient database contains critical information about the Wraith. " * 100
    )
    ...
    try:
        ...
        print(
            f"Large repetitive text: {mckay_stats_large['compression_ratio']:.2f}x compression"
        )
    ...
    print(
        f"  Fountain Codes: {'✅ Working' if ASTRAL_AVAILABLE else '❌ Not Available'}"
    )
