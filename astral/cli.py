...
from .codec import (
    ...
    pack_text_message, pack_voice_message, pack_cmd_message,
    pack_text_with_dict, pack_cmd_batch
)
...
def cmd_simulate(args):
    try:
        ...
        for i in range(0, len(data), atom_size):
            ...
            if len(atom) < atom_size: break
            if random.random() >= args.drop:
                out += atom
        write_bin(args.output, bytes(out))
        print(f"Simulated drop rate {args.drop:.2f}. Input atoms={len(data)//32}, output atoms={len(out)//32}.")
    except Exception as e:
...
def cmd_pack_text(args):
    try:
        blob = pack_text_message(args.text, extra_fountain=args.extra, refine=args.refine)
        write_bin(args.output, blob)
        print(f"Wrote {len(blob)} bytes to {args.output} ({len(blob)//32} atoms). TYPE=TEXT")
    except Exception as e:
...
def cmd_pack_text_dict(args):
    try:
        ...
        blob = pack_text_with_dict(words, args.text, extra_fountain=args.extra, refine=args.refine)
        write_bin(args.output, blob)
        print(f"Wrote {len(blob)} bytes to {args.output} ({len(blob)//32} atoms). TYPE=TEXT with DICT_UPDATE")
    except Exception as e:
...
def cmd_pack_voice(args):
    try:
        ...
        write_bin(args.output, blob)
        print(f"Wrote {len(blob)} bytes to {args.output} ({len(blob)//32} atoms). TYPE=VOICE")
    except Exception as e:
...
def cmd_pack_cmd(args):
    try:
        ...
        blob = pack_cmd_message(cmd, extra_fountain=args.extra, key=key, refine=args.refine)
        write_bin(args.output, blob)
        print(f"Wrote {len(blob)} bytes to {args.output} ({len(blob)//32} atoms). TYPE=CMD")
    except Exception as e:
...
def cmd_pack_cmd_batch(args):
    try:
        ...
        blob = pack_cmd_batch(batch, extra_fountain=args.extra, key=key, key_id=args.key_id, counter=args.counter, refine=args.refine, contact_plan=contact)
        write_bin(args.output, blob)
        print(f"Wrote {len(blob)} bytes to {args.output} ({len(blob)//32} atoms). TYPE=CMD_BATCH")
    except Exception as e:
...
def main(argv=None):
    ...
    p_pack.add_argument("--extra", type=int, default=0, help="extra fountain packets for redundancy")
    p_pack.set_defaults(func=cmd_pack)

    p_unpack = sub.add_parser("unpack", help="unpack atomized binary to JSON (best-effort)")
    ...
    p_sim.add_argument("--drop", type=float, default=0.3, help="probability to drop each atom [0..1]")
    ...
    p_pack_text.add_argument("--refine", type=int, default=0, help="extra REFINE packets")
    p_pack_text.set_defaults(func=cmd_pack_text)

    p_pack_text_dict = sub.add_parser("pack-text-with-dict", help="pack TEXT with a mission lexicon update")
    ...
    p_unpack_voice = sub.add_parser("unpack-voice", help="unpack a VOICE message to WAV")
    ...
    p_pack_cmd_batch = sub.add_parser("pack-cmd-batch", help="pack a batch of time-tagged commands (JSON)")
    ...
    p_pack_cmd_batch.add_argument("--key", help="hex key for HMAC over batch", default=None)
    p_pack_cmd_batch.add_argument("--key-id", type=int, default=0, help="key slot id (0-255)")
    p_pack_cmd_batch.add_argument("--counter", type=int, default=0, help="anti-replay counter")
    p_pack_cmd_batch.add_argument("--contact", help="contact windows 'start-end,start-end' seconds", default=None)
