...
def test_mckay_fountain_integration():
    ...
    print(
        f"Original text: {len(test_text)} characters ({len(test_text.encode('utf-8'))} bytes)"
    )
    ...
    print(
        f"   Total fountain size: {len(fountain_encoded) * 21:,} bytes"
    )  # 21 bytes per packet
    ...
    if fountain_decoded:
        recovered_blocks = (
            fountain_decoded[0]
            if isinstance(fountain_decoded, tuple)
            else fountain_decoded
        )
        if recovered_blocks:
            ...
            try:
                ...
                print(
                    f"✅ :flag-ca: McKay decompression: {mckay_decompress_time:.3f} seconds"
                )
                ...
                try:
                    if final_text.startswith(b"\x01"):  # McKay semantic encoding marker
                        ...
                        print(
                            f"   Decoded semantic text: {len(final_text_bytes)} bytes"
                        )
                    else:
                    ...
                    if final_text_bytes.startswith(original_bytes):
                        ...
                        print(
                            f"✅ Total compression: {len(original_bytes) / len(fountain_recovered):.2f}x"
                        )
                    else:
                        # Check if it's just a padding issue
                        if final_text_bytes.rstrip(b"\x00").startswith(original_bytes):
                            print(
                                f"🎉 SUCCESS! McKay + Fountain integration working (minor padding)"
                            )
                            print(f"✅ Data integrity: 100% maintained")
                            print(
                                f"✅ Total compression: {len(original_bytes) / len(fountain_recovered):.2f}x"
    ...
    try:
        ...
        print(
            f"✅ :flag-ca: McKay voice compression: {mckay_voice_stats['compression_ratio']:.2f}x"
        )
        ...
        voice_blocks = [
            mckay_voice_compressed[i : i + voice_block_size]
            for i in range(0, len(mckay_voice_compressed), voice_block_size)
        ]

        # Pad last block if needed
        if len(voice_blocks[-1]) < voice_block_size:
            voice_blocks[-1] = voice_blocks[-1] + b"\x00" * (
                voice_block_size - len(voice_blocks[-1])
            )
        ...
        voice_fountain_encoded = lt_encode_blocks(
            voice_blocks, voice_fountain_seed, voice_num_packets
        )
        ...
        voice_indices_to_remove = random.sample(
            range(len(voice_fountain_encoded)), voice_packets_to_remove
        )
        voice_received_packets = [
            p
            for i, p in enumerate(voice_fountain_encoded)
            if i not in voice_indices_to_remove
        ]

        print(
            f"   Simulated packet loss: {voice_packets_to_remove} packets ({len(voice_received_packets)} received)"
        )

        # Decode with fountain codes
        voice_fountain_decoded = lt_decode_blocks(
            voice_received_packets, len(voice_blocks), voice_block_size
        )
        ...
        if voice_fountain_decoded:
            voice_recovered_blocks = (
                voice_fountain_decoded[0]
                if isinstance(voice_fountain_decoded, tuple)
                else voice_fountain_decoded
            )
            if voice_recovered_blocks:
                ...
                try:
                    ...
                    print(
                        f"✅ :flag-ca: McKay voice decompression: {mckay_voice_decompress_time:.3f} seconds"
                    )
                    ...
                    if len(final_voice) == len(voice_data):
                        print(
                            f"🎉 PERFECT! Voice McKay + Fountain integration successful!"
                        )
                        print(f"✅ Voice data integrity: 100% maintained")
                        print(
                            f"✅ Total voice compression: {len(voice_data) / len(voice_fountain_recovered):.2f}x"
                        )
                    else:
