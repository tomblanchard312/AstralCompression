...
def compress_video_file(video_path: str):
    ...
    try:
        ...
        try:
            ...
            print(f"  Savings: {file_size - compressed_size:,} bytes ({(file_size - compressed_size) / (1024*1024):.2f} MB)")
    ...
    try:
        ...
        if os.path.exists(original_path):
            ...
            if len(decompressed_data) == len(original_data):
                ...
                print(f"⚠️  Size mismatch: Original {len(original_data):,} vs Decompressed {len(decompressed_data):,}")
