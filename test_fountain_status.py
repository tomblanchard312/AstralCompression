...
def test_fountain_basic():
    ...
    try:
        ...
        block_size = 64
        blocks = [test_data[i:i+block_size] for i in range(0, len(test_data), block_size)]
        # Pad last block if needed
        if len(blocks[-1]) < block_size:
            blocks[-1] = blocks[-1] + b'\x00' * (block_size - len(blocks[-1]))
        
        encoded_blocks = lt_encode_blocks(blocks, 12345, int(len(blocks) * 1.5))  # blocks, seed, num_packets
        print(f"✅ Encoding successful!")
        print(f"Encoded blocks: {len(encoded_blocks)}")
        print(f"Block size: {len(encoded_blocks[0]) if encoded_blocks else 'N/A'} bytes")
        
        # Decode with fountain codes
        print(f"\n🔧 Decoding with fountain codes...")
        decoded_data = lt_decode_blocks(encoded_blocks, 1, 64)  # packets, K, symbol_size
        
        if decoded_data:
            print(f"✅ Decoding successful!")
            # Extract data from tuple (data, fraction)
            actual_data = decoded_data[0] if isinstance(decoded_data, tuple) else decoded_data
...
def test_fountain_voice():
    ...
    try:
        ...
        block_size = 1024
        blocks = [voice_data[i:i+block_size] for i in range(0, len(voice_data), block_size)]
        # Pad last block if needed
        if len(blocks[-1]) < block_size:
            blocks[-1] = blocks[-1] + b'\x00' * (block_size - len(blocks[-1]))
        
        encoded_blocks = lt_encode_blocks(blocks, 54321, int(len(blocks) * 1.2))  # blocks, seed, num_packets
        print(f"✅ Voice encoding successful!")
        print(f"Encoded blocks: {len(encoded_blocks)}")
        
        # Decode with fountain codes
        print(f"🔧 Decoding voice with fountain codes...")
        decoded_voice = lt_decode_blocks(encoded_blocks, len(blocks), 1024)  # packets, K, symbol_size
        
        if decoded_voice:
            print(f"✅ Voice decoding successful!")
            # Extract data from tuple (data, fraction)
            actual_voice = decoded_voice[0] if isinstance(decoded_voice, tuple) else decoded_voice
            ...
            if len(actual_voice) == len(voice_data):
                ...
                if actual_voice[:100] == voice_data[:100] and actual_voice[-100:] == voice_data[-100:]:
