...
def test_voice_parameters():
    ...
    test_configs = [
        {
            "block_size": 256,
            "redundancy": 2.0,
            "description": "Small blocks, high redundancy",
        },
        {
            "block_size": 512,
            "redundancy": 1.8,
            "description": "Medium blocks, high redundancy",
        },
        {
            "block_size": 1024,
            "redundancy": 1.5,
            "description": "Large blocks, medium redundancy",
        },
        {
            "block_size": 2048,
            "redundancy": 1.3,
            "description": "Very large blocks, low redundancy",
        },
        {
            "block_size": 128,
            "redundancy": 2.5,
            "description": "Tiny blocks, very high redundancy",
        },
    ]
    ...
    for config in test_configs:
        ...
        # Split into blocks
        blocks = [
            mckay_compressed[i : i + config["block_size"]]
            for i in range(0, len(mckay_compressed), config["block_size"])
        ]
        ...
        fountain_decoded = lt_decode_blocks(
            fountain_encoded, len(blocks), config["block_size"]
        )
        ...
        if fountain_decoded and fountain_decoded[0]:
            ...
            print(
                f"   ❌ FAILED! Recovery: {fountain_decoded[1] if fountain_decoded else 0:.1%}"
            )
        ...
        indices_to_remove = random.sample(
            range(len(fountain_encoded)), packets_to_remove
        )
        received_packets = [
            p for i, p in enumerate(fountain_encoded) if i not in indices_to_remove
        ]

        fountain_decoded_lossy = lt_decode_blocks(
            received_packets, len(blocks), config["block_size"]
        )
        ...
        if fountain_decoded_lossy and fountain_decoded_lossy[0]:
            ...
            print(
                f"   ❌ FAILED with loss! Recovery: {fountain_decoded_lossy[1] if fountain_decoded_lossy else 0:.1%}"
            )
    ...
    if best_config:
        ...
        blocks = [
            mckay_compressed[i : i + best_config["block_size"]]
            for i in range(0, len(mckay_compressed), best_config["block_size"])
        ]
        if len(blocks[-1]) < best_config["block_size"]:
            blocks[-1] = blocks[-1] + b"\x00" * (
                best_config["block_size"] - len(blocks[-1])
            )
        ...
        fountain_decoded = lt_decode_blocks(
            received_packets, len(blocks), best_config["block_size"]
        )
        ...
        if fountain_decoded and fountain_decoded[0]:
            ...
            try:
                ...
                if len(final_voice) == len(voice_data):
                    ...
                    print(
                        f"✅ Total compression: {len(voice_data) / len(fountain_recovered):.2f}x"
                    )
                    return True
                else:
                    print(
                        f"⚠️  Voice data length mismatch: {len(voice_data)} vs {len(final_voice)}"
                    )
