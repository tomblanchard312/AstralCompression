...
Measured on 100 short mission messages compressed individually:
    ...
    built-in text transform         4,578 bytes
    zstd -19 + trained dict      3,350 bytes

which is why this exists. The win is largest exactly where GistLink is meant to
...
_MISSING = (
    ...
    "Install it with: pip install gistlink[dict]"
)
...
def configured_paths() -> list:
    """
    Dictionary paths from the ``GISTLINK_DICT`` environment variable.
    ...
    every invocation::
        ...
        export GISTLINK_DICT=/etc/gistlink/mission-v3.dict
...
def configured_registry() -> "DictionaryRegistry":
    """Every dictionary named by ``GISTLINK_DICT``, loaded."""
...
def default_dictionary():
    ...
    GistLink works without a dictionary, just less well on short messages.
    """
