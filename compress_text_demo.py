        ...
        for minute in range(0, 60, 5):  # Every 5 minutes
            for sensor_id in range(1, 11):  # 10 sensors
                ...
                status = (
                    "NOMINAL"
                    if value < 150
                    else "WARNING" if value < 180 else "CRITICAL"
                )
...
def compress_text_samples():
    ...
    print(
        f"Original Size: {len(mission_text):,} characters ({len(mission_text.encode('utf-8')):,} bytes)"
    )
    ...
    print(
        f"Original Size: {len(telemetry_text):,} characters ({len(telemetry_text.encode('utf-8')):,} bytes)"
    )
    ...
    print(
        f"Original Size: {len(repetitive_text):,} characters ({len(repetitive_text.encode('utf-8')):,} bytes)"
    )
