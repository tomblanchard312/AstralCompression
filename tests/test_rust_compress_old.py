...
class TestTelemetryCompression:
    ...
    def test_telemetry_performance(self):
        ...
        for _ in range(10):
            ...
            decompressed = ac.decompress_telemetry(compressed, len(data_bytes), n_channels)  # noqa: E501    # noqa: E501# noqa: E501
            # type: ignore
...
class TestCrossCompatibility:
    ...
    def test_telemetry_rust_python_compatibility(self):
        ...
        # Test roundtriploats.append(struct.unpack('>f', data_bytes[i:i + 4])[0])
    ...
    def test_binary_float_rust_python_compatibility(self):
        ...
        # Test that both produce valid output (can't cross-decompress due to different algorithms)
        rust_compressed = ac.compress_binary_float(data_bytes)  # type: ignore  # noqa: E501
        assert len(rust_compressed) > 0
        ...
        # Test roundtrip
        press due to different algorithms)
        rust_compressed = ac.compress_text(text)  # type: ignorepress due to different algorithms)
        rust_compressed = ac.compress_text(text)  # type: ignore  # noqa: E501
        assert len(rust_compressed) > 0press due to different alpress due to different algorithms)
        rust_compressed = ac.compress_text(text)  # type: ignore  # noqa: E501
        assert len(rust_compressed) > 0
        
        
        rust_compressed = ac.compress_text(text)  # typ
    ...
    def test_telemetry_invalid_data_length(self):
        """Test telemetry compression with invalid data length."""
