"""Tests for astral/commands.py - command encoding and anti-replay."""
...
class TestEncodeDecodeCmd:
    def test_reboot_roundtrip(self):
        enc = encode_cmd({"name": "REBOOT"})
        dec = decode_cmd(enc)
        assert dec["name"] == "REBOOT"
    ...
    def test_set_mode_roundtrip(self):
        for mode in ("SAFE", "NORMAL", "SCIENCE"):
            enc = encode_cmd({"name": "SET_MODE", "mode": mode})
            dec = decode_cmd(enc)
...
class TestAntiReplay:
    ...
    def test_tampered_counter_fails_hmac(self):
        ...
        enc[-36] ^= 0x01
        dec = decode_cmd(bytes(enc), key=KEY)
        assert dec["auth_ok"] is False
    ...
    def test_batch_anti_replay(self):
        batch = {
            "policy": {"rollback_on_fail": True},
            "items": [
                {
                    "tai_offset_s": 5,
                    "cmd": {"name": "SET_MODE", "mode": "SCIENCE"},
                },
        ...
        assert dec["auth_ok"] is True
        assert dec.get("counter_ok") is True
        dec_replay = decode_cmd_batch(enc, key=KEY, counter=43)
        assert dec_replay.get("counter_ok") is False


class TestUnauthenticated:
    def test_no_auth_fields_when_no_key(self):
