...
import tempfile
import os
from astral.codec import pack_message, unpack_stream, pack_text_message, pack_cmd_message
...
def test_loss_tolerance():
    ...
    for drop_rate in [0.1, 0.3, 0.5, 0.7]:
        ...
        if drop_rate < 0.6:  # With high redundancy, should recover completely
            assert out["complete"] == True, f"Should recover completely with {drop_rate:.1f} drop rate"
