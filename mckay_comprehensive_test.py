...
def test_text_compression():
    ...
    large_text = (
        "The Ancient database contains critical information about the Wraith. " * 200
    )
    ...
    for name, data, data_type in test_cases:
        ...
        try:
            compressed = integration.compress_and_encode(
                data, data_type, extra_fountain=10
            )
            mckay_stats = integration.mckay_compressor.get_compression_stats()

            print(
                f"{name}: {mckay_stats['compression_ratio']:.2f}x - {mckay_stats['mckay_rating']}"
            )
    ...
    for data_type in data_types:
        for size in test_sizes:
            ...
            try:
                ...
                print(
                    f"{data_type:9} | {size:11} | {compression_time:9.1f} | {stats['compression_ratio']:5.1f}x | {stats['mckay_rating'][:10]}"
                )

            except Exception as e:
                print(
                    f"{data_type:9} | {size:11} | {'ERROR':>9} | {'N/A':>5} | {'ERROR':>10}"
                )
