...
def compress_video_file(video_path: str):
    ...
    try:
        ...
        print(
            f"  ✅ Compression: {stats['compression_ratio']:.2f}x - {stats['mckay_rating']}"
        )
    ...
    best_result = max(
        compression_results, key=lambda x: x["stats"]["compression_ratio"]
    )
    ...
    print(
        f"📊 Compressed Size: {len(best_result['compressed_data']):,} bytes ({len(best_result['compressed_data']) / (1024*1024):.2f} MB)"
    )
    ...
    if best_result["stats"]["compression_ratio"] > 1:
        ...
        print(
            f"⚠️  Note: File expanded during compression (common with already-compressed formats)"
        )
    ...
    try:
        ...
        if compressed_size < file_size:
            ...
            print(
                f"  ⚠️  Expansion: {expansion:,} bytes ({expansion / (1024*1024):.2f} MB)"
            )
            print(
                f"  💡 Note: MP4 files are already highly compressed - McKay's algorithm works best with raw video data"
            )
    ...
    if compressed_path:
        ...
        if os.path.exists(compressed_path):
            ...
            if compressed_size < original_size:
                ...
                print(
                    f"💡 Note: MP4 files are already highly compressed by modern codecs"
                )
                print(
                    f"🔬 McKay's algorithm works best with raw video data or uncompressed formats"
                )
                print(
                    f"🚀 For deep space transmission, consider using raw video formats for maximum compression"
                )
    else:
